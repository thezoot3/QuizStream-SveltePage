<script lang="ts">
	...
	async function submitHandler() {
		...
		if (presetData.correctAnswer === undefined) {
			alert("정답을 선택해주세요.");
			return;
		}
		...
		if (presetData._id) {
			...
			console.log({...presetData as CreateQuiz, subVideoByOptions: subVideoData})
...
{#if presetData}
	<div class="absolute w-full h-full top-0 left-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
		<div class="flex flex-col gap-6 bg-[#222222] p-6 rounded-xl overflow-y-scroll max-h-[80%] shadow-inner">
			<span class="text-2xl font-semibold ">Create/Edit Quiz</span>
			<div class="flex flex-col justify-center items-center gap-4">
				<input type="text" class="text-lg font-medium text-[#999999] p-2 bg-[#2e2e2e] rounded-lg w-full" bind:value={presetData.displayName} placeholder="Quiz Name"/>
				<input type="text" class="text-lg font-medium text-[#999999] p-2 bg-[#2e2e2e] rounded-lg w-full" bind:value={presetData.questionText} placeholder="Question Text"/>
			</div>
			<div class="flex flex-col gap-2">
				...
				<div class="flex justify-between items-center gap-6">
					<select class=" bg-[#2e2e2e] text-gray-300 rounded-lg w-full text-lg p-3" bind:value={presetData.videoId} name="mediaId">
						{#each availableMedia as media}
							<option value={media.videoId}>{media.filename}</option>
						{/each}
					</select>
					<div class="flex gap-1 items-center justify-between">
						<input type="number" class="p-2 w-12 bg-[#2e2e2e] text-gray-300 rounded-lg text-center" bind:value={triggerMinutes} placeholder="Min"/>
						<span class="text-sm font-light text-[#999999]">/</span>
						<input type="number" class="p-2 w-12 bg-[#2e2e2e] text-gray-300 rounded-lg text-center" bind:value={triggerSeconds} placeholder="Sec"/>
					</div>
				</div>
			</div>
			<div class="flex flex-col justify-center items-center gap-2">
				<div class="flex justify-between items-center w-full">
					...
					<button on:click={addOptions} class="text-2xl"><AddIcon/></button>
				</div>
				<div class="w-full grid grid-cols-8 items-center gap-y-3">
					<span class="text-base text-[#999999] col-span-1">Index</span>
					<span class="text-base text-[#999999] col-span-6">Option text</span>
					<span class="text-base text-[#999999] col-span-1">Correct?</span>
					{#each presetData.options || [] as options, index}
						<span class="text-xl font-light text-[#999999] col-span-1">{index + 1}.</span>
						<input bind:value={presetData.options[index]} placeholder="option" class="bg-[#2e2e2e] rounded-lg  p-2 text-lg col-span-6"/>
						<input type="radio" name="answer" value={index} bind:group={presetData.correctAnswer} class="col-span-1">
					{/each}
				</div>
			</div>
			<div class="flex flex-col justify-center items-center gap-2">
				...
				<div class="w-full grid grid-cols-7 items-center gap-y-3">
					...
					{#each presetData.options || [] as options, index}
						<span class="text-xl font-light text-[#999999] col-span-1">{index + 1}.</span>
						<select class=" bg-[#2e2e2e] text-gray-300 rounded-lg w-full text-lg p-3 col-span-6" bind:value={subVideoIdByOptions[index]} name="subVideoId">
			...
			<div class="flex flex-col justify-center items-center gap-2">
				<span class="w-full text-xl font-medium">Submitting Duration & Points</span>
				<div class="flex gap-4 items-center justify-between">
					<input type="number" class="p-2 bg-[#2e2e2e] text-gray-300 rounded-lg" bind:value={presetData.submittingDuration} placeholder="Submitting Duration"/>
					<input type="number" class="p-2 bg-[#2e2e2e] text-gray-300 rounded-lg" bind:value={presetData.points} placeholder="Points"/>
				</div>
			</div>
			<div class="justify-between items-center grid grid-cols-2 gap-6">
				<button class="p-2 bg-[#306844] rounded-lg text-xl text-gray-300 flex gap-2 items-center justify-center" on:click={submitHandler}><SaveIcon/> Save</button>
				<button class="p-2 bg-red-900 rounded-lg text-xl text-gray-300 flex gap-2 items-center justify-center" on:click={cancel}><CancelIcon/> Close</button>
			</div>
		</div>
	</div>
{/if}
